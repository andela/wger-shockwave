...
def analysis(request, username=None):
    '''
    Analysis page
    '''
    users = list(User.objects.all())
    # users = ["emily", "kinuthia"]
    ctx = {
        "users": users,
    }
    ...
    if request.user.is_authenticated():
        return render(request, 'analysis.html', template_data)
    else:
...
def demo_entries(request):
    ...
    if (((not request.user.is_authenticated()
        ...
        messages.success(request,
                         _('We have created sample workout, '
                           'workout schedules, weight logs, '
                           '(body) weight and nutrition plan '
                           'entries so you can better see '
