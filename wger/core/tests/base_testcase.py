...
class WorkoutManagerTestCase(BaseTestCase, TestCase):
    ...
    user_success = 'admin'
    '''
    A list of users to test for success. For convenience,
    a string can be used as well if there is only one user.
    '''

    user_fail = 'test'
    '''
    A list of users to test for failure. For convenience,
    a string can be used as well if there is only one user.
    '''
...
class WorkoutManagerDeleteTestCase(WorkoutManagerTestCase):
    ...
    This assumes the delete action is only triggered with a POST request and
